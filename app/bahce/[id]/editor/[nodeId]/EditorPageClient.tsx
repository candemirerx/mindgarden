...
export default function EditorPageClient({ nodeId }: Props) {
  ...
  const handleSpellCheck = async () => {
    ...
    const textToCheck = hasSelection ? content.substring(selectionStart, selectionEnd) : content;
    if (!textToCheck.trim()) return;
    setIsSpellChecking(true);
    try {
      const response = await fetch(API_ENDPOINTS.spellcheck, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: textToCheck }) });
  ...
  const handleAcceptSpellCheck = () => { setHasChanges(true); setPendingSpellCheck(null); };
  const handleRejectSpellCheck = () => { if (pendingSpellCheck) setContent(pendingSpellCheck.original); setPendingSpellCheck(null); };
  ...
  const handleExportPDF = async () => {
    ...
    doc.setFontSize(18); doc.setFont('helvetica', 'bold'); doc.text(title || 'Başlıksız Not', margin, 25);
    doc.setFontSize(12); doc.setFont('helvetica', 'normal');
    const lines = doc.splitTextToSize(content, maxWidth); doc.text(lines, margin, 40);
  ...
  const handleExportWord = async () => {
    ...
    const doc = new Document({ sections: [{ properties: {}, children: [new Paragraph({ text: title || 'Başlıksız Not', heading: HeadingLevel.HEADING_1 }), ...content.split('\n').map(line => new Paragraph({ children: [new TextRun(line)] }))] }] });
    const blob = await Packer.toBlob(doc);
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `${title || 'not'}.docx`; a.click();
    URL.revokeObjectURL(url);
  };

  const formatLastSaved = () => { if (!lastSaved) return null; const now = new Date(); const diff = Math.floor((now.getTime() - lastSaved.getTime()) / 1000); if (diff < 5) return 'Az önce kaydedildi'; if (diff < 60) return `${diff} sn önce kaydedildi`; return `${Math.floor(diff / 60)} dk önce kaydedildi`; };
      ...
      <header className="bg-white border-b border-stone-300">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <button onClick={handleClose} className="p-2 hover:bg-stone-100 rounded-lg transition-colors" title="Geri Dön"><ArrowLeft size={20} className="text-stone-600" /></button>
            <div className="min-w-0 flex-1"><h1 className="text-base sm:text-lg font-semibold text-stone-800 truncate">{title || 'Başlıksız Not'}</h1></div>
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <button onClick={handleCopy} className={`p-2.5 rounded-lg transition-all ${showCopied ? 'bg-green-100 text-green-600' : 'text-stone-500 hover:bg-stone-100 hover:text-stone-700'}`} title="İçeriği Kopyala">{showCopied ? <Check size={20} /> : <Copy size={20} />}</button>
            <div className="relative" ref={exportMenuRef}>
              <button onClick={() => setShowExportMenu(!showExportMenu)} className="p-2.5 text-stone-500 hover:bg-stone-100 hover:text-stone-700 rounded-lg transition-colors" title="Dışa Aktar"><Download size={20} /></button>
              {showExportMenu && (<div className="absolute right-0 top-full mt-1 bg-white border border-stone-200 rounded-xl shadow-xl py-1.5 min-w-[150px] z-50"><button onClick={handleExportPDF} className="w-full px-4 py-2.5 text-left text-sm text-stone-700 hover:bg-stone-50 transition-colors">PDF olarak indir</button><button onClick={handleExportWord} className="w-full px-4 py-2.5 text-left text-sm text-stone-700 hover:bg-stone-50 transition-colors">Word olarak indir</button></div>)}
            </div>
      ...
      <main className="flex-1 overflow-auto py-4 sm:py-6">
        <div className="max-w-4xl mx-auto px-4 sm:px-0">
          <div className="bg-white shadow-lg min-h-[600px] sm:min-h-[842px] rounded-lg sm:rounded-none">
            <div className="border-b border-stone-200 px-6 sm:px-12 pt-6 pb-4"><input type="text" value={title} onChange={handleTitleChange} placeholder="Başlık" className="w-full text-xl font-semibold text-stone-800 outline-none placeholder:text-stone-300" /></div>
            <div className="px-6 sm:px-12 py-6"><textarea ref={textareaRef} value={content} onChange={handleContentChange} placeholder="İçeriğinizi buraya yazın..." className="w-full min-h-[500px] sm:min-h-[600px] resize-none outline-none text-stone-700 text-base leading-relaxed placeholder:text-stone-300" autoFocus /></div>
          </div>
        </div>
      </main>
      <footer className="bg-white border-t border-stone-300 px-4 sm:px-6 py-2">
        <div className="flex items-center justify-between text-xs text-stone-500">
          <span>{isSaving ? (<span className="flex items-center gap-1"><Loader2 size={12} className="animate-spin" />Kaydediliyor...</span>) : autoSave ? (formatLastSaved() || 'Otomatik kaydetme açık') : hasChanges ? '● Kaydedilmemiş değişiklikler' : 'Kaydedildi'}</span>
          <div className="flex items-center gap-4 sm:gap-6"><span>{content.split(/\s+/).filter(w => w.length > 0).length} kelime</span><span className="hidden sm:inline">{content.length} karakter</span></div>
        </div>
      </footer>
    </div>
  );
